...
@WebServlet("/modifyStory")
public class ModifyControllerStory extends HttpServlet{
	private static final long serialVersionUID = -8195123407449469536L;
	private static final Logger logger = LoggerFactory.getLogger(ModifyControllerStory.class.getName());
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (id == null || id.isEmpty()) {
            ...
            resp.sendRedirect("/farmstory/story/listStory"); // 게시글 목록으로 이동
            return;
        }
		...
		try {
			...
			if (articleDTO == null) {
	            ...
	            resp.sendRedirect("/farmstory/story/listStory"); // 글이 없는 경우 목록으로 리다이렉트
	            return;
	        }
			
	        req.setAttribute("articleDTO", articleDTO);
			
			RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/story/storyModify.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {	
			...
			resp.sendRedirect(req.getContextPath() + "/viewStory?id=" + id);
